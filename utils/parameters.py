...
RERANK_DEFAULTS = {
    "rerankPoolSize": 20,    # how many top fused candidates get sent to the reranker
    "rerankMaxChars": 2000,  # passage text is truncated to this many chars before reranking
    "rerankWeight": 0.4,     # blend reranker refinement while preserving lexical/anchor evidence
}

DEFAULTS = {
    "rrfK": 60,                # RRF smoothing constant
    "vecGap": 0.12,            # adaptive vector score margin around the best hit
    "vectorKeep": 12,           # hard cap for vector-only candidates after retrieval
    "ftsKeep": 10,              # keep the top-N lexical hits regardless of vector score
    "anchorCount": 3,           # only expand context around the top-N ranked candidates
    "symbolAnchorLimit": 25,   # max chunks returned by symbol-name lookup
    "maxSiblingPieces": 3,     # max pieces pulled in per expanded symbol
