...
class ResolvedLookup:
    """A lookup whose matches have all been materialized to code.

    Mirrors the shape of :class:`LookupResult` but adds ``codes`` — the
    reconstructed body for every match (or just the unique exact match).
    """
    ...
    @property
    def matches(self) -> list[SymbolMatch]:
        return self.result.matches

    @property
    def selected(self) -> SymbolCode | None:
        return self.result.selected

    @property
    def limitations(self) -> tuple[str, ...]:
        return self.result.limitations