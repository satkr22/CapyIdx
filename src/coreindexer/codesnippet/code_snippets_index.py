...
import coreindexer.utils.tree_sitter as _utils_tree_sitter
from coreindexer.base.index_d import (
    FileSystem,
    IndexTag,
...
from coreindexer.utils.chunk_utils import tag_to_string
from coreindexer.utils.paths import migrate
from coreindexer.utils.tree_sitter import (
    get_full_language_name,
    get_parser_for_file,
    get_query_for_file,
)
from coreindexer.utils.uri import (
    find_uri_in_dirs,
...
class CodeSnippetsCodebaseIndex(CodebaseIndexer):
    ...
    @staticmethod
    async def get_for_id(id: int, workspace_dirs: list[str], db:sqlite3.Connection,) -> ContextItem:
        db = db
        row = db.execute(
    ...
    @staticmethod
    async def get_all(tag: IndexTag, db:sqlite3.Connection,) -> list[ContextSubmenuItem]:
        db = db
        await CodeSnippetsCodebaseIndex._create_tables(db)
        try:
            rows = db.execute(
