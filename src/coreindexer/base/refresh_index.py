...
from coreindexer.base.index_d import FileStatsMap
...
async def get_add_remove_for_tag(
    ...
    *,
    ...
    saved = get_saved_items_for_tag(tag, only_paths=only_paths, db=db)
...
def get_tags_from_global_cache(
    cache_key: str, artifact_id: str, db:sqlite3.Connection,
...
async def get_compute_delete_add_remove(
    ...
    *,
    ...
    add, remove, last_updated, mark_complete = await get_add_remove_for_tag(
        tag, current_files, db, read_file, only_paths=only_paths)
    ...
    for item in add:
        existing = get_tags_from_global_cache(item.cache_key, tag.artifact_id, db=db)
        if existing:
            add_tag.append(item)
        else:
            compute.append(item)
    ...
    global_cache = GlobalCacheCodeBaseIndex.create(db=db)
...
class GlobalCacheCodeBaseIndex:
    ...
    @classmethod
    def create(cls, db:sqlite3.Connection,) -> GlobalCacheCodeBaseIndex:
        return cls(db)
...
class IndexLock:
    @staticmethod
    def is_locked(db: sqlite3.Connection) -> Optional[dict]:
        db = db
        row = db.execute(
    ...
    @staticmethod
    def lock(dirs: str, db:sqlite3.Connection) -> None:
        db = db
        db.execute(
    ...
    @staticmethod
    def update_timestamp(db:sqlite3.Connection) -> None:
        db = db
        db.execute(
    ...
    @staticmethod
    def unlock(db:sqlite3.Connection) -> None:
        db = db
